...
class ManageApplicationView(View):
    ...
    def post(self, request):
        ...
        user_uuid = None
        if action == "approve":
            user_uuid = UserUniqueId.objects.create(
                uuid = uuid.uuid1()
            )
            application.approved = True
            application.uuid = user_uuid
            application.rejected = False
            messages.success(request, f"Application has been {action}-{user_uuid.uuid}")
        elif action == "reject":
            application.approved = False
            application.rejected = True
            messages.success(request, f"Application has been Rejected")
        application.save()
